...
/**
 * 拼音工具类，该工具类去除静态方法调用，避免多线程时数据异常
 */
public class GetChinesePyUtils {
    ...
    public static final String[] pyNum = new String[]{"abc", "def", "ghi", "jkl", "mno", "pqrs", "tuv", "wxyz"};
     ...
     * 获取拼音
     */
    public String getSpells(String chs, boolean isFirst) {
     ...
     * 获取拼音（含空格）
     */
    public String getSpellsContainSpaces(String chs) {
        if (chs == null || chs.length() == 0) {
            return "";
        }
        String key, value;
        ...
        if (!EmptyUtil.isEmpty(buffer) && buffer.toString().contains(" ")) {
            String[] result = buffer.toString().split(" ");
            StringBuilder bufferNew = new StringBuilder();

            for (int j = 1; j < result.length; j++) {
                bufferNew.append(" ");
                for (int i = j; i < result.length; i++) {
                    bufferNew.append(result[i]);
                }
            }

            return bufferNew.toString();
        }
        return buffer.toString();
    }
     ...
     * 获取拼音对应数字（九宫格键盘）
     */
    public String getSpellsNum(String target) {
        ...
        for (int i = 0; i < target.length(); i++) {
            String curr = target.substring(i, i + 1);
            if (curr.matches("-?[0-9]+.?[0-9]*")) {
                result.append(curr);
            } else {
                for (int j = 0; j < pyNum.length; j++) {
                    if (pyNum[j].contains(target.substring(i, i + 1))) {
                        result.append(j + 2);
                        break;
                    }
                }
     ...
     * 获取拼音对应数字（九宫格键盘/含空格）
     */
    public String getSpellsNumContainSpaces(String target) {
        if (target == null || target.length() == 0) {
            return "";
        }
